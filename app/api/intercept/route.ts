...
export async function POST(request: Request) { const action = await request.json(); const evaluation = evaluateAction(action); const result = { id: crypto.randomUUID(), action, ...evaluation, reasoningSource: 'kairn-policy-engine', status: evaluation.decision === 'allow' ? 'approved' : 'pending' as const }; addAuditRecord(result); return NextResponse.json({ forward: evaluation.decision === 'allow', ...result }, { status: evaluation.decision === 'reject' ? 403 : 200 }); }
